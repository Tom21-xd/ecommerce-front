    ...
    <section className="max-w-5xl mx-auto space-y-8 py-8 px-4 animate-fade-in">
      ...
      {!loading && items.length > 0 && (
        <div className="space-y-8 animate-slide-in-right">
          <div className="rounded-3xl bg-white dark:bg-neutral-900 shadow-xl border-2 border-neutral-200 dark:border-neutral-800 overflow-hidden">
            <div className="divide-y divide-neutral-200 dark:divide-neutral-800">
              {items.map((it) => {
                const deleted = !it.product;
                const seller = it.product?.container?.user;
                const sellerName = seller?.username || seller?.email || "Vendedor";
                return (
                  <div
                    key={it.id}
                  ...
                  >
                    ...
                    <div className="flex-1 w-full">
                      ...
                      <div className="text-xs text-neutral-500 dark:text-neutral-400 mt-1">
                        {deleted ? null : (
                          <>
                            SKU: {it.product?.sku ?? "-"} • Vendedor: {sellerName}
                          </>
                        )}
                      </div>
                      <div
          ...
          <div className="glass rounded-3xl p-6 sm:p-8 border-2 border-neutral-200 dark:border-neutral-700 shadow-xl animate-scale-in">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
              ...
              <Link
                href="/checkout"
                className="inline-flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white px-8 py-4 font-semibold shadow-lg hover:shadow-2xl transition-all duration-300 active:scale-95"
              >
                Continuar al pago
